...
A Tencent-published fork of Claude Code. Speaks the same wire protocol as Claude (stream-json over stdio), so MCP and session resumption behave identically — the daemon dispatches it to the same backend implementation. Independent versioning (`codebuddy --version` is on a 2.x line that doesn't track upstream Claude Code), so the minimum-version gate is relaxed for this entry.

| | |
|---|---|
| Daemon looks for | `codebuddy` |
| Install | npm package `codebuddy` (`npm install -g codebuddy`). See [docs.cloud.tencent.com/document/product/1779](https://cloud.tencent.com/document/product/1779) for the latest official guide. |
| Authentication | `codebuddy` will walk you through Tencent CodeBuddy login on first run. |

### Internal-only forks (enterprise distributions)

Three additional CLIs are recognised so on-prem / corporate environments can register them as runtimes alongside their public counterparts. They share each parent's wire protocol and backend implementation verbatim, but ship under independent version sequences and are not available on the public npm registry. The daemon registers them as separate runtimes (their own row in the Runtimes panel) so the same machine can host both an upstream tool and its internal fork side-by-side.

| Provider key | Family | Daemon looks for | How it's distributed |
|---|---|---|---|
| `claude-internal` | Claude Code | `claude-internal` | Internal package channel — protocol-compatible with Claude Code |
| `codex-internal` | Codex | `codex-internal` | Internal package channel — protocol-compatible with Codex |
| `gemini-internal` | Gemini | `gemini-internal` | Internal package channel — protocol-compatible with Gemini CLI |

If you don't have access to the internal channels, the daemon simply skips these entries during the `PATH` scan — no error, no log noise. Override the binary path per provider with `MULTICA_CLAUDE_INTERNAL_PATH`, `MULTICA_CODEX_INTERNAL_PATH`, or `MULTICA_GEMINI_INTERNAL_PATH` when the binary lives outside the daemon's resolved `PATH`.
