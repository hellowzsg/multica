...
腾讯发布的 Claude Code 同协议分支，走完全相同的 stream-json over stdio 协议，所以 MCP 与会话续接行为与 Claude Code 一致 —— 守护进程会把它派发到同一个 backend 实现。版本号独立成长（`codebuddy --version` 在 2.x 序列上，与上游 Claude Code 不同步），所以这一款在最低版本闸门处放宽处理。

| | |
|---|---|
| 守护进程扫描 | `codebuddy` |
| 安装 | npm 包 `codebuddy`（`npm install -g codebuddy`）。最新指引见 [cloud.tencent.com/document/product/1779](https://cloud.tencent.com/document/product/1779)。 |
| 认证 | 跑一次 `codebuddy`，跟着 CodeBuddy 的登录流程走。 |

### 仅企业内部分发的变体

另外三款 CLI 是为了企业内网 / 私有化场景而识别的：与公开版协议完全相同、复用同一个 backend 实现，但版本号独立、不在公开 npm registry 上分发。守护进程会把它们注册成与公开版**并列的独立运行时**（在 Runtimes 面板各占一行），这样同一台机器上原版与内部分支可以并行使用。

| Provider key | 协议族 | 守护进程扫描 | 分发方式 |
|---|---|---|---|
| `claude-internal` | Claude Code | `claude-internal` | 内部包通道 —— 与 Claude Code 协议一致 |
| `codex-internal` | Codex | `codex-internal` | 内部包通道 —— 与 Codex 协议一致 |
| `gemini-internal` | Gemini | `gemini-internal` | 内部包通道 —— 与 Gemini CLI 协议一致 |

如果你没有这些内部通道的访问权限，守护进程会在 `PATH` 扫描里直接跳过，不报错也不打日志。如果二进制不在守护进程默认 `PATH` 上，可以用环境变量显式指定：`MULTICA_CLAUDE_INTERNAL_PATH`、`MULTICA_CODEX_INTERNAL_PATH`、`MULTICA_GEMINI_INTERNAL_PATH`。
