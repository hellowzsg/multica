...
- **Local skills** — Skills already installed in your local runtime (e.g., `.claude/skills/`, `.opencode/skills/`). For Claude, host-machine skills under `~/.claude/skills/` are **isolated by default** for each agent so a broken host skill on one operator's machine cannot crash a teammate's run (GitHub #3052) — flip the per-agent **Allow host-machine Skills** toggle on if a personal agent intentionally depends on them. Other runtimes still auto-discover their conventional skill paths.
